 ...
 */
export const recordUserJobAction = async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
export const getUserSavedJobs = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (req.user && req.user.id !== userId) {
      throw new BadRequestError('No puedes ver los trabajos guardados de otro usuario');
    }
 ...
 */
export const getUserAppliedJobs = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = parseInt(req.params.userId);
    ...
    // Verificar si el usuario solicitante es el mismo que se quiere consultar
    if (req.user && req.user.id !== userId) {
      throw new BadRequestError('No puedes ver los trabajos aplicados de otro usuario');
    }
