...
import { z } from "zod";
import { insertOccupationSchema, insertUserOccupationSchema } from "../schemas";
 ...
 */
export const importOccupationsFromCSV = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const occupationsCount = await occupationService.importOccupationsFromCSV(
      filePath,
      language
    );
    ...
    res.json({
      success: true,
      message: `Importadas ${occupationsCount} ocupaciones para el idioma ${
        language || "en"
      }`,
    });
  } catch (error) {
 ...
 */
export const getUserOccupations = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
 ...
 */
export const createUserOccupation = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const userOccupation = await occupationService.createUserOccupation(
      userOccupationData
    );
