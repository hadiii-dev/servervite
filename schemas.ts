import { pgTable, text, serial, integer, boolean, timestamp, json, jsonb, doublePrecision, index } from "drizzle-orm/pg-core";
...
export const occupations = pgTable("occupations", {
  ...
  return {
    ...
    preferredLabelIdx: index("occupations_preferred_label_idx").on(table.preferredLabel),
...
export const userOccupations = pgTable("user_occupations", {
  ...
  occupationId: integer("occupation_id").references(() => occupations.id).notNull(),
  ...
  return {
    ...
    occupationIdx: index("user_occupations_occupation_idx").on(table.occupationId),
    // Índice combinado para búsquedas específicas (ocupaciones gustadas por un usuario)
    userLikedIdx: index("user_occupations_user_liked_idx").on(table.userId, table.liked)
  };
});
...
export const userJobs = pgTable("user_jobs", {
  ...
  sentiment: text("sentiment", { enum: ["excited", "interested", "neutral", "doubtful", "negative"] }),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => {
  return {
    // Índice para consultar trabajos guardados/aplicados por usuario
    userActionIdx: index("user_jobs_user_action_idx").on(table.userId, table.action),
...
export const anonymousSessions = pgTable("anonymous_sessions", {
  ...
  return {
    // Índice para búsquedas por ubicación
    geoIdx: index("anonymous_sessions_geo_idx").on(table.latitude, table.longitude),
    // Índice para búsquedas por estado de perfil
    profileCompletedIdx: index("anonymous_sessions_profile_completed_idx").on(table.profileCompleted),
    // Índice para ordenar por fecha de creación
    createdAtIdx: index("anonymous_sessions_created_at_idx").on(table.createdAt)
  };
});

export const sessionJobs = pgTable("session_jobs", {
  id: serial("id").primaryKey(),
  sessionId: text("session_id").references(() => anonymousSessions.sessionId).notNull(),
  jobId: integer("job_id").references(() => jobs.id).notNull(),
  ...
  return {
    ...
    sessionActionIdx: index("session_jobs_session_action_idx").on(table.sessionId, table.action),
    // Índice para optimizar búsquedas de acciones por sesión y trabajo específico
    sessionJobIdx: index("session_jobs_session_job_idx").on(table.sessionId, table.jobId),
    // Índice para ordenar por fecha de creación
...
export const insertUserOccupationSchema = createInsertSchema(userOccupations).omit({
  id: true,
  createdAt: true
});
...
// Types
export type SentimentType = "excited" | "interested" | "neutral" | "doubtful" | "negative";
