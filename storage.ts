import { 
  ...
  occupations, type Occupation, type InsertOccupation,
  userOccupations, type UserOccupation, type InsertUserOccupation,
  jobs, type Job, type InsertJob,
  userJobs, type UserJob, type InsertUserJob,
  anonymousSessions, type AnonymousSession, type InsertAnonymousSession,
  sessionJobs, type SessionJob, type InsertSessionJob
} from "./schemas";
import { db } from "./db";
...
export interface IStorage {
  ...
  getUserOccupationsByUserId(userId: number): Promise<(UserOccupation & { occupationName?: string })[]>;
  createUserOccupation(userOccupation: InsertUserOccupation): Promise<UserOccupation>;
  ...
  updateAnonymousSession(sessionId: string, data: Partial<InsertAnonymousSession>): Promise<AnonymousSession | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getOccupations(searchQuery?: string): Promise<Occupation[]> {
    if (searchQuery) {
      return await db.select().from(occupations).where(
        or(
  ...
  async importOccupationsFromCSV(filePath: string): Promise<number> {
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      const occupationsToInsert = batch.map((record: any) => ({
        ...
        modifiedDate: record.modifiedDate ? new Date(record.modifiedDate) : null,
      ...
      try {
        await db.insert(occupations).values(occupationsToInsert).onConflictDoNothing();
  ...
  async getUserOccupation(id: number): Promise<UserOccupation | undefined> {
    const [userOccupation] = await db.select().from(userOccupations).where(eq(userOccupations.id, id));
    return userOccupation;
  }
  
  async createUserOccupation(userOccupation: InsertUserOccupation): Promise<UserOccupation> {
  ...
  async getUserOccupationsByUserId(userId: number): Promise<(UserOccupation & { occupationName?: string })[]> {
    ...
    try {
      ...
      if (location) {
        if (location === "españa") {
          ...
          const spainKeywords = [
            ...
            "vitoria", "oviedo", "santander", "jerez", "pamplona", "almería", 
            "almeria", "donostia", "san sebastián", "san sebastian", "cartagena"
          ];
          ...
          whereConditions.push(like(jobs.location, `%${location.toLowerCase()}%`));
        }
      }
      ...
      if (orderBy === 'recent') {
        ...
        if (cache.timestamp > now - CACHE_TTL && cache.jobs.length >= limit) {
          console.log(`Usando caché para ${cacheKey} (${cache.jobs.length} trabajos en caché)`);
          
          // Filtramos IDs excluidos si hay
          if (excludeIds.length > 0) {
            result = cache.jobs.filter(job => !excludeIds.includes(job.id))
        ...
        if (totalCount > 0) {
          ...
          const randomOffset = Math.floor(Math.random() * Math.max(1, totalCount - limit));
      ...
      if (skills.length > 0) {
        ...
        if (filteredResults.length < limit * 0.5 && result.length >= filteredResults.length) {
          const remainingCount = limit - filteredResults.length;
          const additionalJobs: Job[] = result
            .filter(job => !filteredResults.some(filtered => filtered.id === job.id))
      ...
      console.log(`Consulta de jobs: orderBy=${orderBy}, filtros=${JSON.stringify({
        category: category || 'none',
        isRemote: isRemote !== undefined ? isRemote : 'any',
        location: location || 'any',
        withSkills: skills.length > 0,
        excludeIds: excludeIds.length > 0 ? `${excludeIds.length} ids` : 'none',
      })}`)
      ...
      try {
        ...
        const backupJobs = await db.select()
          .from(jobs)
          .limit(limit);
  ...
  async getUserJob(id: number): Promise<UserJob | undefined> {
    const [userJob] = await db.select().from(userJobs).where(eq(userJobs.id, id));
    return userJob;
  }
  
  async createUserJob(userJob: InsertUserJob): Promise<UserJob> {
    const [newUserJob] = await db
      .insert(userJobs)
      .values(userJob)
      .returning();
    return newUserJob;
  ...
  async getAnonymousSession(sessionId: string): Promise<AnonymousSession | undefined> {
    const [session] = await db.select().from(anonymousSessions).where(eq(anonymousSessions.sessionId, sessionId));
    return session;
  }
  ...
  async updateAnonymousSession(sessionId: string, data: Partial<InsertAnonymousSession>): Promise<AnonymousSession | undefined> {
    try {
  ...
  async getSessionJob(id: number): Promise<SessionJob | undefined> {
    const [sessionJob] = await db.select().from(sessionJobs).where(eq(sessionJobs.id, id));
    return sessionJob;
  }
